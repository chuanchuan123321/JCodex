...
const browserDetect = require('./lib/browser-detect');
...
/**
 * Make sure a usable browser is available without downloading unless needed.
 * Prefers an existing system Chrome/Chromium, then Playwright's bundled
 * Chromium, and only downloads when neither exists.
 */
function ensureBrowserAvailable() {
  ...
  console.log('⚠️ No system Chrome/Chromium found and Playwright Chromium is missing.');
  console.log('📥 Downloading Chromium (one-time download)...');
  try {
    execSync('npx playwright install chromium', { stdio: 'inherit', cwd: __dirname });
  } catch (e) {
    console.error('❌ Failed to download Chromium:', e.message);
    console.error('Install Google Chrome/Chromium on this machine, or run:');
    console.error('  cd', __dirname, '&& npx playwright install chromium');
    process.exit(1);
  }
}
...
function installPlaywright() {
  console.log('📦 Playwright package not found. Installing...');
  try {
...
/**
 * Snippet that makes every chromium.launch() prefer an existing system
 * Chrome/Chromium. Because require() returns a singleton module, patching
 * playwright.chromium.launch here also affects `const { chromium } =
 * require('playwright')` inside the user's script.
 */
function browserPatchSnippet() {
  return `
const __browserDetect = require('./lib/browser-detect');
const __playwrightMod = require('playwright');
const __systemChrome = __browserDetect.findSystemChrome();
if (__systemChrome) {
  const __pwLaunch = __playwrightMod.chromium.launch.bind(__playwrightMod.chromium);
  __playwrightMod.chromium.launch = (options = {}) => {
    if (!options.executablePath && !options.channel) {
      console.log('💻 Using system browser:', __systemChrome);
      return __pwLaunch({ ...options, executablePath: __systemChrome });
    }
    return __pwLaunch(options);
  };
}
`;
}
...
function wrapCodeIfNeeded(code) {
  ...
  // If it's already a complete script, prepend the browser patch and return
  if (hasRequire && hasAsyncIIFE) {
    return `${browserPatchSnippet()}
${code}`;
  }
...
async function main() {
  ...
  if (!checkPlaywrightInstalled()) {
    ...
    // Package exists; make sure a browser binary is usable too.
    ensureBrowserAvailable();
  }
